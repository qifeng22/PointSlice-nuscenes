...
class SparseHEDNet2D(SparseHEDNet):
    ...
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        self.sed_layersduojiao = nn.ModuleList()
        for idx in range(model_cfg.SED_NUM_LAYERS):
            # self.duojiao = nn.ModuleList(
            #         [post_act_block_sparse_3d(dim, dim, 3, 1, 1, indice_key='spconv1sed', 
            # conv_type='subm')])
            self.sed_layersduojiao.append(post_act_block_sparse_3d(sed_dim, sed_dim, 3, 1, (0,1,1), indice_key=f'spconv1sed_{idx}', 
            conv_type='spconv'))
    ...
    def forward(self, batch_dict):
        ...
        gao = self.sparse_shape[0]
        # jilv = 0
        for layer,duojiao in zip(self.sed_layers,self.sed_layersduojiao):
            x = layer(x)
            x = self.sparse2dto3d(x,gao,batch_size)
            x = duojiao(x)
            x,gao = self.sparse3dto2d(x,batch_size)
